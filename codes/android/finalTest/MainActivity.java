...
import fi.iki.elonen.NanoHTTPD;  // NanoHTTPD 사용
...
public class MainActivity extends AppCompatActivity implements OnInitListener {
        ...
        new Thread(() -> {
            try {
                ...
                Request request = new Request.Builder()
                        .url("http://192.168.4.6:8080/notify")  // Flask 서버에서 알림 수신(pc의 ip주소)
                        .build();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        startNotificationListener();  // 알림 리스너 시작
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    Request request = new Request.Builder()
                            .url("http://192.168.4.6:8080")  // 로컬 서버 주소로 변경 필요(pc의 ip주소)
                            // IPv4 주소
                            .post(requestBody)
                            .build();
    ...
    private void parseJsonResponse(String jsonData) {
        ...
        if (response != null && "success".equals(response.getStatus())) {
            ...
            if (productInfo != null) {
                ...
                // TTS로 읽어주기
                String ttsText = "제품명은 " + productInfo.getName() + ", 제조사는 " + productInfo.getMaker() + ", 조리법" + productInfo.getRecipe() + ".";
        ...
        public Response serve(IHTTPSession session) {
            if (Method.POST.equals(session.getMethod()) && "/notify".equals(session.getUri())) {
                try {
                    ...
                    String json = files.get("postData");  // "postData" 키로 JSON 데이터 추출
                    ...
                    runOnUiThread(() -> {
                        ...
                        Toast.makeText(MainActivity.this, "메시지 수신: " + message.getMessage(), Toast.LENGTH_SHORT).show();
                    });
    ...
    public static class ServerMessage {
        private String message;  // 서버에서 "message" 키로 전달된 값을 저장
